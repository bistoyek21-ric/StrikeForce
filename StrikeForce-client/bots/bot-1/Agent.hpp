...
public:
    Agent(bool training = true,
        ...
        // Obs normalisation (running mean/std)
        obs_mean_ = torch::zeros({num_channels_});
        obs_std_  = torch::ones ({num_channels_});
        obs_count_ = 0;
        if (std::filesystem::exists(backup_dir_ + "/obs_stats.pt")) {
            try {
                std::vector<torch::Tensor> stats;
                torch::load(stats, backup_dir_ + "/obs_stats.pt");
                obs_mean_ = stats[0]; obs_std_ = stats[1];
                obs_count_ = 1;     // avoid division by zero on first update
                log("Loaded obs statistics");
            } catch (...) {}
        }
    ...
    ~Agent() {
        ...
        if (training_ && !backup_dir_.empty()) {
            ...
            std::vector<torch::Tensor> stats = {obs_mean_, obs_std_};
...
private:
    ...
    // ── Obs normalisation ─────────────────────
    torch::Tensor obs_mean_, obs_std_;
    int obs_count_;
    ...
    torch::Tensor to_tensor(const std::vector<float>& raw) {
        ...
        // Normalise per channel with running stats
        for (int c = 0; c < num_channels_; ++c) {
            t[0][c] = (t[0][c] - obs_mean_[c]) / (obs_std_[c] + 1e-8f);
        }
        return t;
    }

    void update_obs_stats(const torch::Tensor& state) {
        // Welford online mean/std per channel
        ++obs_count_;
        for (int c = 0; c < num_channels_; ++c) {
            auto ch = state[0][c];
            float mu_new = ch.mean().item<float>();
            float delta  = mu_new - obs_mean_[c].item<float>();
            obs_mean_[c] = obs_mean_[c] + delta / obs_count_;
            obs_std_[c]  = (obs_std_[c] * (obs_count_ - 1)
                          + (mu_new - obs_mean_[c].item<float>()) * delta) / obs_count_;
            obs_std_[c]  = torch::sqrt(obs_std_[c].abs() + 1e-8f);  // abs for numerical safety
        }
    }
