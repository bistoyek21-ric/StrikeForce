    ...
    bool is_manual() {
        ...
        if (actions_.size() % (T_ / 2) == 0) {
            if (actions_.size()) {
                ...
                std::discrete_distribution<int> dist(v.begin(), v.end());
                std::mt19937 gen(std::random_device{}());
                manual_ = dist(gen);
            }
            else
...
private:
    ...
    // ── State ─────────────────────────────────
    int  cnt_ = 0, T_warmup_ = 16, episode_count_ = 0;
    ...
    void train() {
        ...
        for (int epoch = 0; epoch < num_epochs_; ++epoch) {
            ...
            torch::Tensor entropy = torch::zeros({});
            ...
            for (int t = 0; t < T; ++t) {
                ...
                p_loss += -torch::min(ratio * adv_t, clipped * adv_t);

                // Value loss (MSE on unbounded V)
                v_loss += torch::mse_loss(value, ret_tensor[t]);

                // Entropy bonus
                entropy += -(probs * torch::log(probs + 1e-8f)).sum();
            }
